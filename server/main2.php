...
Route::post("/ajeitar",function(){
    $conn=$GLOBALS["conn"];
    $r=p($conn->query("SELECT CASE WHEN p.descricao LIKE '%governo%' THEN 1 ELSE 0 END AS accuracy, p.descricao, id FROM (SELECT descricao, id FROM post_imagem p WHERE p.privado & 1=0 UNION ALL SELECT NULL AS descricao, id FROM post p WHERE p.privado & 1=0) p ORDER BY accuracy DESC LIMIT 2"));
    echo json_encode($r);
