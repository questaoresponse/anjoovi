...
Route::post("/ajeitar",function(){
    $conn=$GLOBALS["conn"];
    $r=p($conn->query("SELECT CASE WHEN p.descricao LIKE '%governo%' THEN 1 ELSE 0 END AS descricao FROM (SELECT descricao FROM post_imagem) p ORDER BY accuracy DESC LIMIT 2"));
    echo json_encode($r);
