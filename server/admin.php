        ...
        if (($cargo & 2)==2){
            ...
            if ($isCadastro){
                ...
                $conn->prepare("INSERT INTO post(nome,usuario,titulo,subtitulo,texto,imagem,acessos,views_id,id,d,privado) 
                    SELECT nome, usuario, ? AS titulo, ? AS subtitulo, ? AS texto, ? AS imagem, ? AS acessos, ? AS views_id, ? AS id, ? AS d,(? | (CASE WHEN cargo & 1=1 THEN 8 ELSE 0 END)) AS privado FROM user WHERE usuario=?",[$titulo,$subtitulo,$texto,$image,$acessos,$views_id,$id,$d,$permission,$usuario]);
...
Route::post("/admin/playlists_cadastro",function(){
    ...
    if ($type=="info"){
        ...
        if (in_array($data_type,$valid_types)){
            ...
            if ($search!=""){
                ...
                if ($data_type=="post" || $data_type=="post_imagem"){
                    $result=p($conn->prepare("SELECT id,$titulo FROM $data_type WHERE usuario=? AND privado=0 AND LOWER($titulo) LIKE LOWER(?)",[$user,$search]));
                } else {
                    $result=p($conn->prepare("SELECT id,titulo FROM $data_type WHERE usuario=? AND privado=0 AND LOWER($titulo) LIKE LOWER(?)",[$user,$search]));
                }
            } else {
                if ($data_type=="post" || $data_type=="post_imagem"){
                    $result=p($conn->prepare("SELECT id,$titulo AS titulo FROM $data_type WHERE usuario=? AND privado=0",[$user]));
                } else {
                    $result=p($conn->prepare("SELECT id,titulo FROM $data_type WHERE usuario=? AND privado=0",[$user]));
                }
            }
        ...
        $conn=$GLOBALS["conn"];
        $cargo=$GLOBALS["cargo"];
        if (($cargo & 4)==0 && isset($_POST["permission"]) && $_POST["permission"]==2) return response()->json(["result"=>"false"]);
        $permission=isset($_POST["permission"]) && $_POST["permission"]==2 ? 2 : 0;
        $data_type=request("data_type");
        $valid_types=["post","post_imagem","post_musica"];
        if (in_array($data_type,$valid_types)){
            $posts=json_decode(request("posts"));
            $list_posts=to_list($posts);
            $num=p($conn->prepare("SELECT COUNT(*) AS num FROM $data_type WHERE id IN ($list_posts) AND usuario=?",[$user]))[0]["num"];
            if ($num==count($posts)){
                $titulo=request("titulo");
                $d=get_d();
                ...
                $conn->prepare("INSERT INTO playlist(usuario,views_id,id,titulo,d,tipo,posts,privado) SELECT usuario, ? AS views_id, ? AS id, ? AS titulo, ? AS d, ? AS tipo, ? AS posts, (? | (CASE WHEN cargo & 1=1 THEN 8 ELSE 0 END)) AS privado FROM user WHERE usuario=?",[$views_id,$id,$titulo,$d,$data_type,$posts,$permission,$user]);
                insert_views($conn,$user,"playlist",$views_id,$id);
                add_n_posts($user,$conn);
                response()->json(["result"=>"true"]);
            } else {
                response()->json(["result"=>"false"]);
...
Route::post("/admin/playlists_lista",function(){
    ...
    function gpa($type){
        ...
        if (($cargo & 2)==2){
            ...
            if ($type=="pesquisa"){
                $s=$conn->prepare("SELECT * FROM playlist WHERE usuario=? AND LOWER(titulo) LIKE LOWER(?) || LOWER(d) LIKE LOWER(?) ORDER BY id DESC LIMIT $n,10",[$user,$search,$search]);
                $num=$conn->prepare("SELECT COUNT(*) AS num FROM playlist WHERE usuario=? AND LOWER(titulo) LIKE LOWER(?) || LOWER(d) LIKE LOWER(?)",[$user,$search,$search]);
            } else {
                $s=$conn->prepare("SELECT * FROM playlist WHERE usuario=? ORDER BY id DESC LIMIT $n,10",[$user]);
                $num=$conn->prepare("SELECT COUNT(*) AS num FROM playlist WHERE usuario=?",[$user]);
            }
        }
        $r=p($s);
        $num=p($num)[0]["num"];
...
Route::post("/admin/metricas",function(){
    ...
    if (request("type")=="info"){
        ...
        $end=microtime(true);
        $t2=($end - $start) * 1000;
        echo json_encode(["posts"=>$p,"total_u"=>$total_u,"usuario"=>$usuario,"t1"=>$t1,"t2"=>$t2]);
    } else {
