                ...
                <div className="titulo-video">{post.titulo.map((line,i)=>{
                    return <>
                        {line.map((titulo,index)=>{
                            return titulo.length>0 && titulo[0]=="@" ? <Link className='tag' key={String(i)+String(index)} to={"/@"+encodeURIComponent(titulo.slice(1))}>{titulo + + ( line.length-1>index ? " " : "" )}</Link> : titulo + ( line.length-1>index ? " " : "" )
                        })}
                        <br></br>
                    </>
                })}</div>
