...
if ($url=="/"){
    ...
    if ($result->num_rows==0){
        if ($url=="admin"){
            include(__DIR__ . "/templates/admin_login/admin_init.php");
        } else{
            include(__DIR__ . "/templates/erro/404.html");
        }
    }else{
        $r=[];
        ...
        switch ($url){
            case 'admin':
                include(__DIR__ . "/templates/admin_barra/cadastro_usuario.php");
                    break;
            case 'admin/noticias_cadastro':
                include(__DIR__ . "/templates/admin_noticias_cadastro/noticias_cadastro.php");
                break;
            case 'admin/noticias_lista':
                include(__DIR__ . "/templates/admin_noticias_lista/noticias_lista.php");
                break;
            case 'admin/config_cadastro':
                include(__DIR__ . "/templates/admin_config/config_cadastro.html");
                break;
            case 'admin/sair':
    ...
    if ($result->num_rows>0){
        ...
        include(__DIR__ . "/templates/posts/post.php");
    } else {
        header("HTTP/1.0 404 Not Found");
        include(__DIR__ . "/templates/erro/404.html");
    }
}else if ($url=="reload"){
    $conn = new mysqli("127.0.0.1:3306", "root",null);
    verificar($conn,"ip");
